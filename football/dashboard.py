...
# total aggregates for categorize players
...
df_total['Team'] = df_total['Team'].str[:3]

df = pd.merge(df,df_total, how='left',on=['Team','Player'])

# Categorize players
df['Player_Category'] = 'Receiver'
df.loc[(df['Total_Passing_Yds'] > df['Total_Rushing_Yds']) & (df['Total_Passing_Yds'] > df['Total_Receiving_Yds']),
       'Player_Category'] = 'Quarterback'
df.loc[(df['Total_Rushing_Yds'] > df['Total_Passing_Yds']) & (df['Total_Rushing_Yds'] > df['Total_Receiving_Yds']),
...
games_played = games_won.add(games_lost, fill_value=0).astype(int).reset_index().rename({'index':'NFL_Team',
                                                                                         'count':'max_week'},axis=1)
...
if page =='Home':
    ...
    if page1_selection =='Passing':
        # render_passing_section(
        #     passing_top_df=passing_top_df,
        #     real_teams=real_teams,
        #     team_selected=team_selected,
        #     team_color=team_color,
        #     highlight_team=highlight_team,
        #     fig_passing=fig_passing
        # )
        with middle3:
