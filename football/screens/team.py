...
def render(data):
    ...
    # Create three columns for legend + three charts as in original
    ...
    if not passing.empty:
        passing_mapping_domain = players_all
        passing_legend = alt.Chart(passing).mark_bar(opacity=0).encode(
            ...
            color=alt.Color('Player:N',
                            scale=alt.Scale(domain=passing_mapping_domain, range=None),
                            legend=alt.Legend(title="Passing", orient='left', direction='vertical')
                            ),
